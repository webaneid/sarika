 ...
 */
function sarika_post_meta( int $post_id = 0 ) : void {
 ...
 */
function sarika_get_company_name() : string {
 ...
 */
function sarika_get_category_section_data() {
	// Get ACF sub-fields.
...
function sarika_pagination( ?WP_Query $query = null ) : void {
	...
	echo '<nav class="pagination my-8" aria-label="' . esc_attr__( 'Pagination', 'sarika' ) . '">';
 ...
 */
function sarika_open_graph_meta_tags() {
	...
	if ( has_post_thumbnail( $post_id ) ) {
		...
		$image    = wp_get_attachment_image_src( $image_id, 'sarika-card-large' );
		if ( $image ) {
			$og_image        = $image[0];
