...
/**
 * Sarika Theme Updater Class
 */
class Sarika_Updater {
	...
	private function get_remote_version() {
		...
		// Get download URL from assets (sarika-x.x.x.zip).
		$package_url = '';
		if ( ! empty( $data['assets'] ) && is_array( $data['assets'] ) ) {
			foreach ( $data['assets'] as $asset ) {
				if ( isset( $asset['name'] ) && strpos( $asset['name'], 'sarika-' ) === 0 && strpos( $asset['name'], '.zip' ) !== false ) {
	...
	public function add_update_checker_notice() {
		...
		if ( version_compare( $this->current_version, $remote_version['version'], '<' ) ) {
			...
			<div class="notice notice-info is-dismissible">
				<p>
					<strong><?php esc_html_e( 'Sarika Theme Update Available!', 'sarika' ); ?></strong><br>
					<?php
					printf(
						/* translators: 1: new version, 2: current version, 3: update URL */
						esc_html__( 'Version %1$s is available. You have version %2$s.', 'sarika' ) . ' <a href="%3$s" class="update-link">' . esc_html__( 'Update now', 'sarika' ) . '</a>',
	...
	public function add_update_action_link( $actions, $theme ) {
		...
		if ( version_compare( $this->current_version, $remote_version['version'], '<' ) ) {
			...
			$actions['update'] = sprintf(
				...
				esc_attr( sprintf( __( 'Update %s now', 'sarika' ), 'Sarika' ) ),
				__( 'Update Available', 'sarika' )
			);
		}
	...
	public static function force_update_check() {
		delete_transient( 'sarika_update_check' );
 ...
 */
function sarika_add_manual_update_check() {
	add_action( 'admin_notices', function() {
		$screen = get_current_screen();
		if ( 'themes' === $screen->base && isset( $_GET['sarika_force_check'] ) ) {
			Sarika_Updater::force_update_check();
			?>
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e( 'Update check completed!', 'sarika' ); ?></p>
			</div>
			<?php
		}

		// Debug: Show update transient data (remove after debugging).
		if ( 'themes' === $screen->base && isset( $_GET['sarika_debug'] ) ) {
			$updates = get_site_transient( 'update_themes' );
			echo '<div class="notice notice-info"><pre>';
			echo 'Current version: ' . wp_get_theme()->get( 'Version' ) . "\n";
			echo 'Update data: ';
			print_r( isset( $updates->response['sarika'] ) ? $updates->response['sarika'] : 'No update data' );
			echo '</pre></div>';
		}
	});
}
add_action( 'admin_init', 'sarika_add_manual_update_check' );
