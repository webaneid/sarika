 ...
 */
function sarika_toggle_switch( string $name, bool $checked = false, string $label = '' ) : string {
	$checked_attr = $checked ? ' checked' : '';
	$output = '<label class="sarika-toggle-wrapper">';
	$output .= '<span class="sarika-toggle">';
	$output .= '<input type="checkbox" name="' . esc_attr( $name ) . '"' . $checked_attr . '>';
	$output .= '<span class="sarika-toggle-slider"></span>';
	$output .= '</span>';
	if ( $label ) {
