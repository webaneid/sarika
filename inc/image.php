 ...
 */
function sarika_register_image_sizes() : void {
	...
	add_image_size( 'sarika-square', 300, 300, true );
	add_image_size( 'sarika-square-sm', 150, 150, true );

	// News sizes - 16:9 aspect ratio.
	add_image_size( 'sarika-news-sm', 480, 270, true );
	...
	add_image_size( 'sarika-og', 1200, 630, true );
}
add_action( 'after_setup_theme', 'sarika_register_image_sizes' );

/**
 * Enable WebP support in WordPress media uploads.
 */
function sarika_enable_webp_upload( array $mimes ) : array {
	$mimes['webp'] = 'image/webp';
	return $mimes;
}
add_filter( 'upload_mimes', 'sarika_enable_webp_upload' );

/**
 * Fix WebP thumbnail display in media library.
 */
function sarika_webp_display( array $result, string $path ) : array {
...
add_filter( 'wp_check_filetype_and_ext', 'sarika_webp_display', 10, 2 );
 ...
 */
function sarika_generate_webp_on_upload( array $metadata ) : array {
	...
	sarika_create_webp_image( $file_path, $metadata );
	...
	if ( ! empty( $metadata['sizes'] ) && is_array( $metadata['sizes'] ) ) {
		...
		foreach ( $metadata['sizes'] as $size => $size_data ) {
			...
			sarika_create_webp_image( $size_path, $size_data );
		}
	}

	return $metadata;
}
add_filter( 'wp_generate_attachment_metadata', 'sarika_generate_webp_on_upload' );
 ...
 */
function sarika_create_webp_image( string $file_path, array $metadata = array() ) : bool {
 ...
 */
function sarika_get_webp_srcset( int $attachment_id, string $size = 'full' ) : string {
 ...
 * @return void
 */
function sarika_picture_webp( int $attachment_id, string $size = 'full', array $attr = array() ) : void {
	$image_src    = wp_get_attachment_image_src( $attachment_id, $size );
	$image_srcset = sarika_get_webp_srcset( $attachment_id, $size );
 ...
 */
function sarika_custom_image_sizes( array $sizes ) : array {
	return array_merge(
		$sizes,
		array(
			'sarika-square'    => __( 'Square (300x300)', 'sarika' ),
			'sarika-square-sm' => __( 'Square Small (150x150)', 'sarika' ),
			'sarika-news-sm'   => __( 'News Small (480x270)', 'sarika' ),
			'sarika-news-md'   => __( 'News Medium (800x450)', 'sarika' ),
			'sarika-news-lg'   => __( 'News Large (1024x576)', 'sarika' ),
			'sarika-news-xl'   => __( 'News XLarge (1600x900)', 'sarika' ),
			'sarika-hero'      => __( 'Hero Banner (1920x1080)', 'sarika' ),
			'sarika-og'        => __( 'Social Sharing (1200x630)', 'sarika' ),
		)
	);
}
add_filter( 'image_size_names_choose', 'sarika_custom_image_sizes' );
 ...
 */
function sarika_fix_admin_thumbnail( $image, int $id, $size, bool $icon, $attr ) : string {
...
add_filter( 'wp_get_attachment_image', 'sarika_fix_admin_thumbnail', 10, 5 );
 ...
 */
function sarika_fix_media_grid_thumbnail( array $response, $attachment, array $meta ) : array {
	$upload_dir = wp_upload_dir();
...
add_filter( 'wp_prepare_attachment_for_js', 'sarika_fix_media_grid_thumbnail', 10, 3 );
