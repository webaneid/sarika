 ...
 */
function sarika_get_option_group( string $group_key ) : array {
 ...
 */
function sarika_prepare_link( $link ) : array {
	...
	return array(
		...
		'title'  => sanitize_text_field( $link['title'] ?? __( 'Read More', 'sarika' ) ),
 ...
 */
function sarika_get_about_company_data() : array {
	$group = sarika_get_option_group( 'ane_about_company' );

	return array(
		'name'        => sanitize_text_field( $group['ane_name'] ?? '' ),
		'description' => sanitize_text_field( $group['ane_description'] ?? '' ),
		'link'        => sarika_prepare_link( $group['ane_link'] ?? array() ),
	);
}
 ...
 * Contact info data.
 */
function sarika_get_address_contact_data() : array {
	$group = sarika_get_option_group( 'ane_address_contact' );
 ...
 */
function sarika_get_whatsapp_link( string $number, string $message = '' ) : string {
	if ( empty( $number ) ) {
		return '';
	}
 ...
 * Social links list.
 */
function sarika_get_social_links() : array {
	$group = sarika_get_option_group( 'ane_social_media' );
	...
	$map = array(
		'ane_facebook'        => array( 'slug' => 'facebook', 'label' => __( 'Facebook', 'sarika' ) ),
		'url_threads'         => array( 'slug' => 'threads', 'label' => __( 'Threads', 'sarika' ) ),
		'ane_tiktok'          => array( 'slug' => 'tiktok', 'label' => __( 'TikTok', 'sarika' ) ),
		'ane_instagram'       => array( 'slug' => 'instagram', 'label' => __( 'Instagram', 'sarika' ) ),
		'ane_whatsapp_chanel' => array( 'slug' => 'whatsapp-channel', 'label' => __( 'WhatsApp Channel', 'sarika' ) ),
		'ane_x'               => array( 'slug' => 'x', 'label' => __( 'X', 'sarika' ) ),
	);
	...
	foreach ( $map as $field => $meta ) {
		...
		$links[] = array(
			...
			'label' => $meta['label'],
			'url'   => $url,
			'icon'  => sarika_get_social_icon( $meta['slug'] ),
		);
	}
	...
	if ( $number ) {
		$links[] = array(
			'slug'  => 'whatsapp',
			'label' => __( 'WhatsApp', 'sarika' ),
			'url'   => sarika_get_whatsapp_link( $number, $message ),
			'icon'  => sarika_get_social_icon( 'whatsapp' ),
		);
	}
 ...
 */
function sarika_get_footer_logo_html() : string {
 ...
 */
function sarika_get_copyright_years() : string {
	$latest  = get_posts(
		array(
			'fields'         => 'ids',
			'posts_per_page' => 1,
 ...
 */
function sarika_get_company_display_name() : string {
 ...
 */
function sarika_get_social_icon( string $slug ) : string {
	$icons = array(
