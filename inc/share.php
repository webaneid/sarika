 ...
 */
function sarika_get_share_text( int $post_id = 0, string $platform = 'default' ) : string {
 ...
 */
function sarika_get_share_links( int $post_id = 0 ) : array {
	$post_id = $post_id ? $post_id : get_the_ID();
	$url     = get_permalink( $post_id );

	// Get formatted text for each platform.
	$whatsapp_text = sarika_get_share_text( $post_id, 'whatsapp' );
	$telegram_text = sarika_get_share_text( $post_id, 'telegram' );
	$x_text        = sarika_get_share_text( $post_id, 'x' );
 ...
 * @param int $post_id Post ID.
 */
function sarika_share_buttons( int $post_id = 0 ) {
	$post_id = $post_id ? $post_id : get_the_ID();
	$links   = sarika_get_share_links( $post_id );
	...
	<div class="share-buttons">
		<div class="share-buttons__label">
			<?php esc_html_e( 'Share this article:', 'sarika' ); ?>
		</div>
		<div class="share-buttons__list">
			<?php if ( isset( $links['facebook'] ) ) : ?>
				<a href="<?php echo esc_url( $links['facebook'] ); ?>"
				   ...
				   aria-label="<?php esc_attr_e( 'Share on Facebook', 'sarika' ); ?>">
			...
			<?php if ( isset( $links['x'] ) ) : ?>
				<a href="<?php echo esc_url( $links['x'] ); ?>"
				   ...
				   target="_blank"
				   rel="noopener noreferrer"
				   aria-label="<?php esc_attr_e( 'Share on X (Twitter)', 'sarika' ); ?>">
					<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
			...
			<?php if ( isset( $links['whatsapp'] ) ) : ?>
				<a href="<?php echo esc_url( $links['whatsapp'] ); ?>"
				   ...
				   target="_blank"
				   rel="noopener noreferrer"
				   aria-label="<?php esc_attr_e( 'Share on WhatsApp', 'sarika' ); ?>">
					<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
			...
			<?php if ( isset( $links['telegram'] ) ) : ?>
				<a href="<?php echo esc_url( $links['telegram'] ); ?>"
				   ...
				   target="_blank"
				   rel="noopener noreferrer"
				   aria-label="<?php esc_attr_e( 'Share on Telegram', 'sarika' ); ?>">
					<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
