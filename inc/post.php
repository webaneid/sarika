...
/**
 * Accessible pagination styled for Sarika.
 *
...
function ane_post_pagination( ?WP_Query $wp_query = null, bool $echo = true ) {
	...
	$links = paginate_links(
		array(
			...
			'mid_size'  => 2,
			'prev_next' => true,
			'prev_text' => '<span class="ane-pagination__icon" aria-hidden="true">&lsaquo;</span><span class="screen-reader-text">' . esc_html__( 'Previous', 'sarika' ) . '</span>',
			'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next', 'sarika' ) . '</span><span class="ane-pagination__icon" aria-hidden="true">&rsaquo;</span>',
		)
	);
	...
	$markup = '<nav class="ane-pagination" aria-label="' . esc_attr__( 'Pagination', 'sarika' ) . '"><ul class="ane-pagination__list">';
 ...
 */
function sarika_breadcrumbs( array $args = array() ) : void {
	$defaults = array(
		'wrap_before' => '<nav class="ane-breadcrumbs" aria-label="' . esc_attr__( 'Breadcrumbs', 'sarika' ) . '"><ol>',
		'wrap_after'  => '</ol></nav>',
	...
	$items[] = sprintf(
		...
		esc_url( home_url( '/' ) ),
		esc_attr__( 'Go to homepage', 'sarika' ),
		$home_icon,
		esc_html__( 'Home', 'sarika' )
	);
 ...
 */
function sarika_get_primary_category( ?int $post_id = null ) : ?WP_Term {
 ...
 */
function sarika_post_category( ?int $post_id = null, string $class = '' ) : void {
	$category = sarika_get_primary_category( $post_id );
 ...
 */
function sarika_relative_time( string|int|null $time = null ) : string {
	...
	if ( $diff < HOUR_IN_SECONDS ) {
		...
		return sprintf(
			...
			_n( '%s minute ago', '%s minutes ago', $minutes, 'sarika' ),
	...
	if ( $diff < DAY_IN_SECONDS ) {
		...
		return sprintf(
			...
			_n( '%s hour ago', '%s hours ago', $hours, 'sarika' ),
	...
	if ( $diff < WEEK_IN_SECONDS ) {
		...
		return sprintf(
			...
			_n( '%s day ago', '%s days ago', $days, 'sarika' ),
	...
	if ( $diff < MONTH_IN_SECONDS ) {
		...
		return sprintf(
			...
			_n( '%s week ago', '%s weeks ago', $weeks, 'sarika' ),
	...
	if ( $diff < YEAR_IN_SECONDS ) {
		...
		return sprintf(
			...
			_n( '%s month ago', '%s months ago', $months, 'sarika' ),
	...
	return sprintf(
		...
		_n( '%s year ago', '%s years ago', $years, 'sarika' ),
 ...
 * @param string|int|null $time Unix timestamp or date string.
 */
function sarika_post_time( string|int|null $time = null ) : void {
	$time = $time ?: get_the_time( 'U' );
	...
	printf(
		...
		esc_attr( $datetime ),
		esc_html( sarika_relative_time( $time ) )
	);
}
 ...
 */
function sarika_get_post_image( ?int $post_id = null, string $size = 'medium', ?string $fallback_url = null ) : ?string {
	$post_id = $post_id ?: get_the_ID();
 ...
 * @param string|null $fallback_url Fallback image URL.
 */
function sarika_post_thumbnail( string $size = 'medium', array $attr = array(), ?string $fallback_url = null ) : void {
	$defaults = array(
		'loading' => 'lazy',
 ...
 */
function sarika_get_excerpt( int $word_limit = 20, ?int $post_id = null ) : string {
	$post_id = $post_id ?: get_the_ID();
 ...
 * @param int|null $post_id    Post ID.
 */
function sarika_post_excerpt( int $word_limit = 20, ?int $post_id = null ) : void {
	echo '<p class="post-excerpt">' . esc_html( sarika_get_excerpt( $word_limit, $post_id ) ) . '</p>';
}
 ...
 */
function sarika_get_reading_time( ?int $post_id = null ) : string {
	...
	return sprintf(
		...
		_n( '%d min read', '%d mins read', $reading_time, 'sarika' ),
		$reading_time
	);
}
 ...
 * @param int|null $post_id Post ID.
 */
function sarika_reading_time( ?int $post_id = null ) : void {
	$reading_time = sarika_get_reading_time( $post_id );
	if ( $reading_time ) {
 ...
 */
function sarika_get_related_posts( ?int $post_id = null, int $limit = 6 ) : WP_Query {
	$post_id = $post_id ?: get_the_ID();
 ...
 */
function sarika_get_category_posts( ?int $post_id = null, int $limit = 6 ) : WP_Query {
	$post_id = $post_id ?: get_the_ID();
	$category = sarika_get_primary_category( $post_id );
 ...
 */
function sarika_get_newest_posts( int $limit = 6 ) : WP_Query {
	$args = array(
		'posts_per_page' => $limit,
		'orderby'        => 'date',
		'order'          => 'DESC',
 ...
 */
function sarika_get_views( ?int $post_id = null ) : string {
	$post_id = $post_id ?: get_the_ID();
	...
	return sprintf(
		...
		_n( '%s view', '%s views', $count, 'sarika' ),
 ...
 */
function sarika_set_views( ?int $post_id = null ) : bool {
 ...
 */
function sarika_get_trending_posts( int $limit = 6 ) : WP_Query {
	$args = array(
		'posts_per_page' => $limit,
 ...
 */
function sarika_get_trending_tags( int $limit = 10 ) : array {
	global $wpdb;
 ...
 */
function sarika_column_views( array $defaults ) : array {
	$defaults['sarika_post_views'] = __( 'Views', 'sarika' );
	return $defaults;
}
add_filter( 'manage_posts_columns', 'sarika_column_views' );
 ...
 */
function sarika_custom_column_views( string $column_name, int $post_id ) : void {
	if ( 'sarika_post_views' === $column_name ) {
		echo esc_html( sarika_get_views( $post_id ) );
	}
}
add_action( 'manage_posts_custom_column', 'sarika_custom_column_views', 5, 2 );

/**
 * Enqueue post views AJAX script for cached sites.
 */
function sarika_postview_cache_enqueue() : void {
	if ( ! is_single() ) {
		return;
	}
	...
	wp_enqueue_script(
		'sarika-postviews-cache',
		SARIKA_URI . '/js/postviews-cache.js',
		array( 'jquery' ),
		'1.0.0',
		true
	);
...
add_action( 'wp_enqueue_scripts', 'sarika_postview_cache_enqueue' );

/**
 * AJAX handler to increment views for cached sites.
 */
function sarika_increment_views_ajax() : void {
...
add_action( 'wp_ajax_postviews', 'sarika_increment_views_ajax' );
add_action( 'wp_ajax_nopriv_postviews', 'sarika_increment_views_ajax' );
