 ...
 */
function sarika_render_home_sections() : void {
 ...
 */
function sarika_section_query_args( array $args = array() ) : array {
	$defaults = array(
		'post_type'      => 'post',
		'posts_per_page' => 4,
		'post_status'    => 'publish',
...
/**
 * Check if current page has header overlay enabled.
 *
 * @return bool True if header should overlay content, false otherwise.
 */
function sarika_has_header_overlay() : bool {
	if ( ! function_exists( 'get_field' ) ) {
		return false;
	}

	// Check ACF field for current page/post
	$overlay_enabled = get_field( 'ane_header_overlay' );

	return (bool) $overlay_enabled;
}

/**
 * Add body class for header overlay pages.
 *
 * @param array $classes Body classes.
 * @return array Modified body classes.
 */
function sarika_add_header_overlay_body_class( array $classes ) : array {
	if ( sarika_has_header_overlay() ) {
		$classes[] = 'has-header-overlay';
	}

	return $classes;
}
add_filter( 'body_class', 'sarika_add_header_overlay_body_class' );
 ...
 */
function sarika_hex2rgb( string $colour ) {
	if ( '#' === $colour[0] ) {
 ...
 * - ane-warna-utama      → --sarika-color-primary
 * - ane-warna-utama-2    → --sarika-color-secondary
 * - ane-warna-text       → --sarika-color-body
 * - ane-warna-terang     → --sarika-color-light
 * - ane-warna-gelap      → --sarika-color-dark
 * - ane-warna-alternatif → --sarika-color-accent
 * - ane-warna-putih      → --sarika-color-white
 *
 * Di-inject ke wp_head dengan priority 999 untuk override default tokens.
 */
function sarika_custom_colors_css() {
	...
	// Start CSS output.
	$css = '<style id="sarika-custom-colors">' . "\n";
	$css .= ':root {' . "\n";
	...
	foreach ( $color_map as $acf_field => $css_var_name ) {
		...
		if ( $color ) {
			...
			// Hex color.
			$css .= "\t--sarika-color-{$css_var_name}: {$color};\n";

			// RGB variant.
			$rgb = sarika_hex2rgb( $color );
			if ( $rgb ) {
				$css .= "\t--sarika-color-{$css_var_name}-rgb: {$rgb};\n";
			}
		}
	}
...
add_action( 'wp_head', 'sarika_custom_colors_css', 999 );
 ...
 */
function sarika_custom_colors_admin_css() {
	// Color mapping: ACF field name => CSS variable name.
	$color_map = array(
		'ane-warna-utama'      => 'primary',
		'ane-warna-utama-2'    => 'secondary',
	...
	// Start CSS output.
	$css = '<style id="sarika-custom-colors-admin">' . "\n";
	$css .= ':root {' . "\n";
...
add_action( 'admin_head', 'sarika_custom_colors_admin_css', 999 );

/**
 * Helper function: Generate CSS variables dari ACF color fields.
 *
 * @return string CSS dengan custom color variables (plain CSS, no wrapper).
 */
function sarika_get_custom_colors_css() {
	$color_map = array(
		'ane-warna-utama'      => 'primary',
		'ane-warna-utama-2'    => 'secondary',
		'ane-warna-text'       => 'body',
	...
	foreach ( $color_map as $acf_field => $css_var_name ) {
		...
		if ( $color ) {
			$has_custom_colors = true;
			$rgb = sarika_hex2rgb( $color );
			$css .= "--sarika-color-{$css_var_name}: {$color} !important;\n";
			if ( $rgb ) {
				$css .= "--sarika-color-{$css_var_name}-rgb: {$rgb} !important;\n";
			}
		}
	}

	return $has_custom_colors ? $css : '';
}

/**
 * Inject ACF custom colors ke Block Editor via wp_add_inline_style().
 *
 * WordPress wraps editor CSS dengan :root :where(.editor-styles-wrapper),
 * jadi kita TIDAK perlu manual wrap selector. WordPress handles it automatically.
 */
function sarika_custom_colors_block_editor_css() {
	// Register dummy style handle (no actual file).
	wp_register_style( 'sarika-editor-colors', false );
	wp_enqueue_style( 'sarika-editor-colors' );

	// Get ACF custom colors CSS.
	$css = sarika_get_custom_colors_css();

	if ( $css ) {
		// Wrap dalam :root - WordPress akan auto-wrap dengan :where(.editor-styles-wrapper).
		$wrapped_css = ":root {\n" . $css . "}\n";
		wp_add_inline_style( 'sarika-editor-colors', $wrapped_css );
	}
}
add_action( 'enqueue_block_editor_assets', 'sarika_custom_colors_block_editor_css', 999 );

/**
 * Register ACF Options Page untuk Theme Customization.
 *
 * Location: Sarika Setup → Theme Customization
 */
function sarika_register_acf_options_customization() {
	...
	acf_add_options_sub_page(
		array(
			'page_title'  => __( 'Theme Customization', 'sarika' ),
			'menu_title'  => __( 'Customization', 'sarika' ),
			'menu_slug'   => 'sarika-customization',
...
// add_action( 'acf/init', 'sarika_register_acf_options_customization' );
 ...
 * Functions sarika_custom_colors_css() dan sarika_custom_colors_admin_css()
 ...
 * Location: wp_head
 */
function sarika_gtm_header_content() {
	$script = get_field( 'ane_ga_header', 'option' );
	if ( ! empty( $script ) ) {
		echo $script; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'wp_head', 'sarika_gtm_header_content', 10 );
 ...
 * Location: wp_head
 */
function sarika_sc_header_content() {
	$script = get_field( 'ane_sc_header', 'option' );
	if ( ! empty( $script ) ) {
		echo $script; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'wp_head', 'sarika_sc_header_content', 11 );
 ...
 * Location: wp_head
 */
function sarika_metapixel_header_content() {
	$script = get_field( 'ane_metapixel_header', 'option' );
	if ( ! empty( $script ) ) {
		echo $script; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'wp_head', 'sarika_metapixel_header_content', 12 );
 ...
 */
function sarika_meta_sdk_script() {
	$script = get_field( 'ane_metasdk_body', 'option' );
	if ( ! empty( $script ) ) {
		echo $script; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'wp_body_open', 'sarika_meta_sdk_script' );
 ...
 * Location: wp_footer
 */
function sarika_gtm_footer_content() {
	$script = get_field( 'ane_ga_footer', 'option' );
	if ( ! empty( $script ) ) {
		echo $script; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'wp_footer', 'sarika_gtm_footer_content', 100 );
 ...
 */
function sarika_facebook_sdk_script() {
...
add_action( 'wp_body_open', 'sarika_facebook_sdk_script', 1 );
 ...
 */
function sarika_is_facebook_comments_enabled() {
 ...
 */
function sarika_get_facebook_app_id() {
 ...
 */
function sarika_get_facebook_comments_num_posts() {
 ...
 */
function sarika_register_facebook_comments_fields() {
	// Check if ACF function exists.
	...
	acf_add_local_field_group(
		array(
			'key'      => 'group_sarika_facebook_comments',
			'title'    => __( 'Facebook Comments', 'sarika' ),
			'fields'   => array(
				// Enable Facebook Comments.
				array(
					'key'           => 'field_ane_facebook_comments_enable',
					'label'         => __( 'Enable Facebook Comments', 'sarika' ),
					'name'          => 'ane_facebook_comments_enable',
					'type'          => 'true_false',
					'instructions'  => __( 'Enable to replace WordPress native comments with Facebook Comments Social Plugin.', 'sarika' ),
					'ui'            => 1,
					'default_value' => 0,
				),
				// Facebook App ID.
				array(
					...
					'label'             => __( 'Facebook App ID', 'sarika' ),
					'name'              => 'ane_facebook_app_id',
					'type'              => 'text',
					'instructions'      => __( 'Enter your Facebook App ID. Get it from: https://developers.facebook.com/apps/', 'sarika' ),
					'placeholder'       => '352619118625171',
					'prepend'           => __( '<strong>IMPORTANT:</strong> After entering App ID, make sure your website domain is added in Facebook App Settings → Basic → App Domains. Example: yoursite.com', 'sarika' ),
				...
				array(
					...
					'label'             => __( 'Language / Locale', 'sarika' ),
					'name'              => 'ane_facebook_comments_locale',
					'type'              => 'select',
					'instructions'      => __( 'Select language for Facebook Comments plugin.', 'sarika' ),
					'choices'           => array(
				...
				array(
					...
					'label'             => __( 'Number of Comments', 'sarika' ),
					'name'              => 'ane_facebook_comments_num_posts',
					'type'              => 'number',
					'instructions'      => __( 'Number of comments to display (default: 10).', 'sarika' ),
					'default_value'     => 10,
					'min'               => 1,
					'max'               => 100,
					'step'              => 1,
...
add_action( 'acf/init', 'sarika_register_facebook_comments_fields' );
 ...
 */
function sarika_facebook_comments_og_tags() {
	...
	$fb_app_id = sarika_get_facebook_app_id();
	?>
...
add_action( 'wp_head', 'sarika_facebook_comments_og_tags', 5 );
 ...
 */
function sarika_facebook_comments_debug() {
	...
	Is Enabled Check: <?php echo sarika_is_facebook_comments_enabled() ? 'TRUE' : 'FALSE'; ?>
...
add_action( 'wp_footer', 'sarika_facebook_comments_debug', 999 );
 ...
 */
function sarika_acf_custom_avatar( $avatar, $id_or_email, $size, $default, $alt ) {
	$user = false;
	...
	$square_size = $size <= 150 ? 'sarika-square-sm' : 'sarika-square'; // 150x150 or 300x300.
	...
	if ( is_array( $custom_avatar ) ) {
		...
		} elseif ( isset( $custom_avatar['sizes']['sarika-square-sm'] ) ) {
			$avatar_url = $custom_avatar['sizes']['sarika-square-sm']; // Fallback to small square.
		} elseif ( isset( $custom_avatar['sizes']['sarika-square'] ) ) {
			$avatar_url = $custom_avatar['sizes']['sarika-square']; // Fallback to large square.
...
add_filter( 'get_avatar', 'sarika_acf_custom_avatar', 10, 5 );
