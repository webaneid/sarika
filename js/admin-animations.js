...
(function() {
	...
	function initRippleEffect() {
		...
		buttons.forEach(function(button) {
			button.addEventListener('click', function(e) {
				...
				ripple.classList.add('sarika-ripple');
	...
	function initNoticeAnimations() {
		...
		notices.forEach(function(notice) {
			notice.classList.add('sarika-notice-animate');
	...
	function initProgressBarAnimations() {
		const progressBars = document.querySelectorAll('.sarika-breakdown-progress__fill');
	...
	function addSkeletonLoader(container) {
		container.classList.add('sarika-skeleton-loading');

		setTimeout(function() {
			container.classList.remove('sarika-skeleton-loading');
			container.classList.add('sarika-skeleton-loaded');
		}, 300);
	}
		...
		links.forEach(function(link) {
			link.addEventListener('click', function(e) {
				...
				if (href && href.includes('wp-admin') && !href.includes('customize.php')) {
					...
					// Fade out animation
					document.body.classList.add('sarika-page-exit');
	...
	window.sarikaAddSkeletonLoader = addSkeletonLoader;
