...
// Content options
$content_width = $attrs['ane_content_width'] ?? 'medium';
...
// Build section class
$class_name = 'sarika-block-text';
$class_name .= ' sarika-block-text--align-' . esc_attr( $alignment );
...
$container_class = 'sarika-block-text__container';
...
// Build content class
$content_class = 'sarika-block-text__content';
$content_class .= ' text-' . esc_attr( $alignment );

if ( $content_width === 'medium' ) {
	$content_class .= ' content-width-medium';
} else {
	$content_class .= ' content-width-full';
}

// Title class and style
$title_class = 'sarika-block-text__title title-' . esc_attr( $title_size );
$title_style = '';
if ( $title_color ) {
	if ( str_starts_with( $title_color, '#' ) || str_starts_with( $title_color, 'rgb' ) ) {
		$title_style = 'color: ' . esc_attr( $title_color ) . ';';
	} else {
		$title_style = 'color: var(--sarika-color-' . esc_attr( $title_color ) . ');';
	}
}

// Tagline class and style
$tagline_class = 'sarika-block-text__tagline title-tagline title-' . esc_attr( $tagline_size );
$tagline_style = '';
if ( $tagline_color ) {
	if ( str_starts_with( $tagline_color, '#' ) || str_starts_with( $tagline_color, 'rgb' ) ) {
		$tagline_style = 'color: ' . esc_attr( $tagline_color ) . ';';
	} else {
		$tagline_style = 'color: var(--sarika-color-' . esc_attr( $tagline_color ) . ');';
	}
}
...
if ( $description_color ) {
	if ( str_starts_with( $description_color, '#' ) || str_starts_with( $description_color, 'rgb' ) ) {
		...
		$desc_style = 'color: var(--sarika-color-' . esc_attr( $description_color ) . ');';
	}
}
?>
	...
	<div class="container">
		<div class="<?php echo esc_attr( $container_class ); ?>" <?php echo $container_style ? 'style="' . esc_attr( $container_style ) . '"' : ''; ?>>
			<div class="<?php echo esc_attr( $content_class ); ?>">
				...
				<?php if ( $description ) : ?>
					<p class="sarika-block-text__description desc" <?php echo $desc_style ? 'style="' . esc_attr( $desc_style ) . '"' : ''; ?>>
				...
				<?php if ( $button_url || $button2_url ) : ?>
					<div class="sarika-block-text__buttons">
						<?php if ( $button_url ) : ?>
							<a
