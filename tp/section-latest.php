...
$section_title = function_exists( 'get_sub_field' ) ? get_sub_field( 'section_title' ) : __( 'Latest', 'sarika' );
$posts_per     = function_exists( 'get_sub_field' ) ? (int) get_sub_field( 'posts_per_page' ) : 4;

$query = new WP_Query(
	sarika_section_query_args(
		array(
	...
	<div class="max-w-6xl mx-auto px-4">
		<div class="flex items-center justify-between mb-6">
			...
			<a class="text-sm uppercase tracking-wide hover:underline" href="<?php echo esc_url( get_permalink( get_option( 'page_for_posts' ) ) ); ?>">
				<?php esc_html_e( 'View all', 'sarika' ); ?>
			</a>
		</div>
		<div class="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
			...
			if ( $query->have_posts() ) :
				...
				echo '<p class="text-gray-500">' . esc_html__( 'No posts available yet.', 'sarika' ) . '</p>';
			endif;
			?>
		</div>
	</div>
