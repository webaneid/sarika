		...
		<div class="grid lg:grid-cols-2fr-1fr gap-8 lg:divide-x divide-gray-200">
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'single-post lg:pr-8 min-w-0' ); ?> itemscope itemtype="https://schema.org/NewsArticle">
				...
				<div class="single-post__category-wrapper mb-4">
					<?php sarika_post_category( null, 'single-post__category' ); ?>
					<span class="single-post__separator">•</span>
					<?php sarika_reading_time(); ?>
				</div>
				...
				<?php if ( has_post_thumbnail() ) : ?>
					<div class="single-post__featured-image-wrapper -mx-4 lg:mx-0 mb-6">
						<figure class="single-post__featured-image" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
							...
							sarika_post_thumbnail( $image_size, array(
								'class'    => 'w-full h-auto',
								'itemprop' => 'url',
							) );
							?>
				...
				<div class="single-post__content" itemprop="articleBody">
					...
					wp_link_pages(
						array(
							'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'sarika' ),
							'after'  => '</div>',
						)
					);
					?>
				...
				if ( comments_open() || get_comments_number() || sarika_is_facebook_comments_enabled() ) {
					comments_template();
				}
				?>
				...
				<div itemprop="publisher" itemscope itemtype="https://schema.org/Organization">
					<meta itemprop="name" content="<?php echo esc_attr( sarika_get_company_name() ); ?>">
			...
			<aside class="sidebar lg:pl-8">
				...
				<?php else : ?>
					<div class="sidebar-placeholder p-6 bg-gray-100 rounded-lg text-center">
						<p class="text-sm text-gray-600"><?php esc_html_e( 'No widgets in sidebar. Add from WP Admin.', 'sarika' ); ?></p>
					</div>
				<?php endif; ?>
			</aside>
		</div>
	...
	<div class="single-footer">
		<div class="single-footer__grid">
			...
			<div class="single-footer__column">
				...
				esc_html_e( 'Related Posts', 'sarika' );
				?></h3>
				<?php
				$related_query = sarika_get_related_posts( get_the_ID(), 6 );
				if ( $related_query->have_posts() ) :
					...
					echo '<p class="single-footer__empty">' . esc_html__( 'No related posts found.', 'sarika' ) . '</p>';
				endif;
				?>
			</div>
			...
			<div class="single-footer__column">
				<?php
				$category = sarika_get_primary_category( get_the_ID() );
				$category_title = $category ? $category->name : __( 'Category', 'sarika' );
				?>
				<h3 class="single-footer__title"><?php echo esc_html( $category_title ); ?></h3>
				<?php
				$category_query = sarika_get_category_posts( get_the_ID(), 6 );
				if ( $category_query->have_posts() ) :
					...
					echo '<p class="single-footer__empty">' . esc_html__( 'No posts found in this category.', 'sarika' ) . '</p>';
				endif;
				?>
			</div>
			...
			<!-- Newest Posts -->
			<div class="single-footer__column">
				<h3 class="single-footer__title"><?php esc_html_e( 'Newest Posts', 'sarika' ); ?></h3>
				<?php
				$newest_query = sarika_get_newest_posts( 6 );
